...
p.add_argument('--model',type=str, default='models/27_2_512_2048.pth')
p.add_argument('--double', action='store_true')
p.add_argument('--sr', type=int, default=44100) # this doesn't work
args = p.parse_args()
start_time = time.time()
if len(args.input) != 2:
    quit('You need two tracks!')
print('loading tracks...', end=' ')
wave1,_ = librosa.load(args.input[0], mono=False,sr=args.sr)
wave2,_ = librosa.load(args.input[1], mono=False,sr=args.sr)
if args.align:
